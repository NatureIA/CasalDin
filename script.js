...
const LOCAL_NOTES_KEY = "casadin_notas_locais_v1";
...
async function loadData() {
  ...
  try {
    ...
    state.records = [...normalizeRows(rows), ...loadLocalNotes()];
    populateYearFilter();
    renderDashboard();
    setSyncState("online");
    hideError();
...
/* =========================================================
   Inclusão de nota por OCR — registro local integrado ao portal
   ========================================================= */
function bindNoteInclusion() {
  ...
  submitButton.addEventListener("click", saveExtractedNote);
...
function saveExtractedNote() {
  const responsible = document.getElementById("noteResponsavel").value;
  const category = document.getElementById("noteCategoria").value;
  const description = document.getElementById("noteDescricao").value.trim();
  const amount = parseMoney(document.getElementById("noteValor").value);
  const paymentMethod = document.getElementById("noteFormaPagamento").value.trim();

  if (!responsible || !category || !description || !amount || amount <= 0) {
    return showNoteStatus("Confira responsável, categoria, descrição e valor antes de confirmar.", "error");
  }

  const now = new Date();
  const storedNote = {
    localId: `nota-${Date.now()}`,
    ...
    valor: amount,
    formaPagamento: paymentMethod || "Não informado",
    modeloPagamento: "À vista",
    parcelasRestantes: 0,
    frequencia: "",
  };
  ...
  state.records.push(normalizeLocalNote(storedNote, state.records.length + 1));
  populateYearFilter();
  renderDashboard();
  showNoteStatus("Nota incluída com sucesso no CasaDin.", "success");
  window.setTimeout(closeNoteModal, 850);
}
...
function readStoredNotes() {
  try {
    ...
    console.warn("Falha ao ler notas locais", error);
    return [];
  }
}

function loadLocalNotes() {
  return readStoredNotes().map((note, index) => normalizeLocalNote(note, 1000000 + index));
}

function normalizeLocalNote(note, id) {
  const date = note.dataISO ? new Date(note.dataISO) : new Date();
  return {
    id,
    data: Number.isNaN(date.getTime()) ? new Date() : date,
    dataOriginal: Number.isNaN(date.getTime()) ? "" : dateFormatter.format(date),
    responsavel: titleCase(note.responsavel || "Não informado"),
    tipo: "Despesa",
    categoria: titleCase(note.categoria || "Outros"),
    descricao: note.descricao || "Sem descrição",
    valor: Number(note.valor) || 0,
    formaPagamento: titleCase(note.formaPagamento || "Não informado"),
    modeloPagamento: "À vista",
    parcelasRestantes: 0,
    frequencia: "",
    raw: { origem: "Inclusão de nota" },
  };
}
